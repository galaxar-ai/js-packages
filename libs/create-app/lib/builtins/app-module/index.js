...
async function updateServerConfig_(app, options) {
  ...
  await fs.writeJson(serverConfigFile, serverConfig, {
    spaces: 4
  });
...
module.exports = async (app, options) => {
  ...
  await steps.requireFilesExist_(app, options.workingPath, ['package.json', 'conf/server.default.json']);
  const rootTemplatePath = getTemplatePath(options.appMode, 'root');
  await steps.copyFilesFromTemplate_(app, rootTemplatePath, options.workingPath, options, true);
