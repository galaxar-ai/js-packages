...
const _types = require('@genx/types');
const _Feature = _interop_require_default(require('../Feature'));
const _LibModule = _interop_require_default(require('../LibModule'));
...
const _default = {
    ...
    load_: async (app, entries) => {
        return (0, _utils.batchAsync_)(entries, async (config, name) => {
            let options = { env: app.env, logLevel: app.options.logLevel, ...config.options };
            let appPath;
            if (config.npmModule) {
