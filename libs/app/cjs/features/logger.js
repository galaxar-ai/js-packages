...
const _default = {
    ...
    load_: function (app, config, name) {
        ...
        config = app.featureConfig(config, { schema: { transport: { type: 'object', optional: true } } }, name);
        const options = {
            nestedKey: 'payload',
        ...
        const logger = pino({ level: app.options.logLevel === 'verbose' ? 'debug' : app.options.logLevel, ...options });
        if (isAppLogger) {
