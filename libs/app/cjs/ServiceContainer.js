...
const _jsonc = _interop_require_wildcard(require('@galaxar/jsonc'));
...
const _defaultOpts = _interop_require_default(require('./defaultOpts'));
const _AsyncEmitter = _interop_require_default(require('./helpers/AsyncEmitter'));
...
const configOverrider = (defConf, envConf) => {
    ...
    if (defServiceGroup || envServiceGroup) {
        defServiceGroup &&
            _utils._.each(defServiceGroup, (servicesMap, serviceName) => {
                serviceGroup[serviceName] = servicesMap;
            });
        envServiceGroup &&
            _utils._.each(envServiceGroup, (servicesMap, serviceName) => {
                serviceGroup[serviceName] = { ...serviceGroup[serviceName], ...servicesMap };
            });
    }
    const ret = { ..._def, ..._env };
...
class ServiceContainer extends _AsyncEmitter.default {
    async start_() {
        ...
        this._featureRegistry = { '*': this._getFeatureFallbackPath() };
        this.features = {};
        this.services = {};
        if (this.options.loadConfigFromOptions) {
            ...
            this.configLoader = this.options.disableEnvAwareConfig
                ? new _jsonc.default(
                      new _jsonc.JsonConfigProvider(
                          _nodepath.default.join(this.configPath, this.options.configName + '.json')
                      ),
                      this
                  )
                : _jsonc.default.createEnvAwareJsonLoader(
                      this.configPath,
    ...
    logException(level, error, summary) {
        this.log(
            level,
            (summary ? summary + '\n' : '') + error.message,
            _utils._.pick(error, ['name', 'status', 'code', 'info', 'stack', 'request'])
        );
        return this;
    }
    ...
    sanitize(config, typeInfo, name, category) {
        try {
            return _types.Types.sanitize(config, { type: 'object', ...typeInfo }, undefined, name);
        } catch (err) {
            throw new _types.InvalidConfiguration(err.message, this, { category, name });
        }
    }
    _getConfigVariables() {
        const processInfo = {
        ...
        return { app: this, env: this.env, process: processInfo };
    }
    _getFeatureFallbackPath() {
        return [_nodepath.default.resolve(__dirname, 'features'), this.featuresPath];
    }
    ...
    _loadFeature(feature) {
        ...
        if (this._featureRegistry.hasOwnProperty(feature)) {
            ...
            let found = _utils._.findLast(searchingPath, (p) =>
                FILE_EXT.find((ext) => {
    ...
    constructor(name, options) {
        ...
        this.name = name;
        this.options = { ..._defaultOpts.default, ...options };
        this.env = this.options.env;
        this.workingPath = this.options.workingPath
            ? _nodepath.default.resolve(this.options.workingPath)
            : process.cwd();
