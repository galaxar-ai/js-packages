...
const Routable = (T)=>{
    class _class extends T {
        ...
        _createEngine() {
            try {
                let Engine = (0, _utils.esmCheck)(require(`./engines/${this.options.engine}`));
                return new Engine(this);
            } catch (err) {
                if (err.code === 'MODULE_NOT_FOUND') {
                    throw new _types.InvalidArgument(`Failed to load engine "${this.options.engine}". ${err.message}`);
                }
                throw err;
            }
        }
         ...
         */ constructor(name, options){
            super(name, {
                ..._defaultOpts.defaultRoutableOpts,
                ...options
            });
            /**
            ...
            this.router.use(this.router.middleware((ctx, next)=>{
                ctx.appModule = this;
                return next();
            }));
