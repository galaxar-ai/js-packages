...
const _defaultSyntax = /*#__PURE__*/ _interop_require_default(require("./defaultSyntax"));
...
class ConfigLoader {
     ...
     */ async load_(variables) {
        ...
        if (oldData) {
            this.data = _lodash.default.defaults(this.data, oldData);
        }
        return this.data;
    }
    /**
     ...
     */ postProcess(variables) {
        ...
        const interpolateElement = (coll, key, val)=>{
            ...
            } else if (_lodash.default.isPlainObject(val) || _lodash.default.isArray(val)) {
                queue.push(val);
            }
        };
        let offset = 0;
        while(queue.length > offset){
            ...
            if (_lodash.default.isPlainObject(node)) {
                _lodash.default.forOwn(node, (value, key)=>{
     ...
     */ constructor(configProvider, logger, postProcessors){
         ...
         * Post processors
         * @private
         */ this.postProcessors = postProcessors != null ? _lodash.default.defaultsDeep(postProcessors, _defaultSyntax.default) : _defaultSyntax.default;
    }
}
